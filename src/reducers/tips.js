...
export default function tips (state = defaultState, action) {
  switch (action.type) {
    ...
    case REQUEST_UPDATE_TIP:
      return {
        ...state,
        isSaving: true
      }
    case TIP_WAS_UPDATED:
      return {
        ...state,
        isSaving: false,
        tips: state.tips.map(
          tip => (tip.id === action.tip.id ? action.tip : tip)
        )
      }
    case TIP_WAS_NOT_UPDATED:
      return {
        ...state,
        isSaving: false
      }
    ...
    case TIP_WAS_DELETED:
      return {
        ...state,
        isDeleting: false,
        items: state.items.filter(item => !item.tip.endsWith(action.id))
      }
